...
export async function storeTokens(
  ...
  console.log(
    `[Tokens] storeTokens called: userId=${userId}, connectedEmail=${connectedEmail || 'undefined'}`,
  );
...
export function getTokens(userId: string): Promise<StoredTokens | null> {
  return withDb(async (sql) => {
    ...
    const row = rows[0];
    console.log(`[Tokens] Found tokens: connected_email=${row.connected_email || 'null'}`);
...
export function updateConnectedEmail(userId: string, email: string): Promise<void> {
  return withDb(async (sql) => {
    await sql`
      UPDATE oauth_tokens
